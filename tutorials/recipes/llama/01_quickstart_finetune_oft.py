...
"""
Quickstart: Finetune Llama 3.2 1B with OFT (Orthogonal Fine-Tuning)

OFT learns block-diagonal orthogonal rotations applied to the input of
linear layers. Unlike LoRA which adds a low-rank residual (W' = W + BA),
OFT applies a multiplicative orthogonal transform (y = W @ R @ x),
preserving the angular structure of the pretrained weight space.

Reference: https://arxiv.org/abs/2306.07280

Usage:
    Single GPU with OFT:
        torchrun --nproc_per_node=1 01_quickstart_finetune_oft.py \
            --pretrained-checkpoint /path/to/megatron/checkpoint

    Multiple GPUs (automatic data parallelism):
        torchrun --nproc_per_node=8 01_quickstart_finetune_oft.py \
            --pretrained-checkpoint /path/to/megatron/checkpoint
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Finetune Llama 3.2 1B with OFT",
...
def main() -> None:
    """Run Llama 3.2 1B finetuning with OFT."""
    args = parse_args()

    # Load the base finetune configuration with OFT instead of LoRA
    oft = OFT(block_size=32)
    config = llama32_1b_peft_config(peft_scheme=oft)
    ...
    output_dir = os.path.join(os.getcwd(), "nemo_experiments", "llama32_1b_oft")
    ...
    config.logger.wandb_exp_name = "llama32_1b_oft"
    ...
    # === Adjust OFT hyperparameters ===
    # Larger block_size = more parameters per block, potentially better quality but slower
    # config.peft.block_size = 64

    # Use number of blocks instead of block_size (mutually exclusive)
    # config.peft.r = 16

    # Constrained OFT - projects rotations to epsilon-ball around identity
    # config.peft.coft = True
    # config.peft.eps = 6e-5

    # Share parameters across all blocks (fewer total params)
    # config.peft.block_share = True

    # Multiplicative dropout (randomly replace blocks with identity during training)
    # config.peft.module_dropout = 0.1

    # === Full supervised finetuning (no OFT) ===
