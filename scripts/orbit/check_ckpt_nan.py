...
"""Check a trusted Megatron torch_dist checkpoint for unusual values.

This diagnostic materializes every tensor on CPU and may require at least the
full logical checkpoint size in host memory. Checkpoint metadata may use Python
serialization, so never run it against an untrusted checkpoint.
"""
...
DEFAULT_CKPT = (
    Path(os.environ.get("MEGATRON_CKPT_ROOT", Path.cwd() / "checkpoints")) / "Moonlight-16B-A3B-INT4" / "iter_0000000"
)
