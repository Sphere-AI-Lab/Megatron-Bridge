...
_EXPERT_INT4_TRIPLET_KEY_RE = re.compile(
...
def build_config(args):
    ...
    config.logger.wandb_exp_name = (
        f"qwen3_30b_a3b_qoft_int4_bs{args.block_size}_tp{args.tp}_ep{args.ep}"
    )
    return config
...
def _is_int4_triplet_key(key: str) -> bool:
    return (
        _DENSE_INT4_TRIPLET_KEY_RE.match(key) is not None
        or _EXPERT_INT4_TRIPLET_KEY_RE.match(key) is not None
    )
...
def install_int4_checkpoint_load_patches(group_size: int) -> None:
    ...
    def _int4_load_model_state_dict(module, state_dict, strict=True):
        ...
        if missing or unexpected:
            ...
            if missing:
                details.append(
                    "missing=" + ", ".join(missing[:20]) + (" ..." if len(missing) > 20 else "")
                )
            if unexpected:
                details.append(
                    "unexpected=" + ", ".join(unexpected[:20]) + (" ..." if len(unexpected) > 20 else "")
                )
            raise RuntimeError(
                "Unexpected non-INT4 state_dict mismatch during Qwen3 MoE INT4 load: "
                + " | ".join(details)
            )
