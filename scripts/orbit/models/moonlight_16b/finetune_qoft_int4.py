...
def main() -> None:
    ...
    if args.memory_smoke_test:
        ...
        print_rank_0("Running Moonlight INT4 memory smoke test: 1 training step, no checkpoint save, no evaluation.")
    ...
    output_dir = args.output_dir or os.path.join(os.getcwd(), "nemo_experiments", "moonlight_16b_qoft_int4")
    ...
    config.logger.wandb_exp_name = f"moonlight_16b_qoft_int4_bs{args.block_size}_tp{args.tp}_ep{args.ep}"
    ...
    def _int4_load_model_state_dict(model_module, state_dict, strict=True):
        ...
        if missing or unexpected:
            ...
            raise RuntimeError(
                "Unexpected non-INT4 state_dict mismatch during Moonlight INT4 load: " + " | ".join(details)
            )
