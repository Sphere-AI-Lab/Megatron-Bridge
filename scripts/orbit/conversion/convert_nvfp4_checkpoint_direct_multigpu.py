...
def _to_empty_if_meta_device_cpu(module, *, device=None, recurse=True):
    return _orig_to_empty_if_meta_device(module, device=torch.device("cpu"), recurse=recurse)
...
class _SpillBucket:
    ...
    def __init__(self, path, prealloc_bytes):
        ...
        self.storage = torch.UntypedStorage.from_file(str(path), shared=True, nbytes=prealloc_bytes)
...
def _bucketed_spill_tensor(self, key, tensor):
    ...
    bucket_name = f"layer_{int(match.group(1)):03d}" if match is not None else "misc"
    ...
    with bucket.lock:
        ...
        view = torch.empty(0, dtype=tensor.dtype).set_(bucket.storage, elem_offset, tensor.shape, tensor.stride())
        view.copy_(tensor)
...
def _log_stage_done(message: str, start_time: float, *, extra: Optional[str] = None) -> None:
    suffix = f" | {extra}" if extra else ""
    print_rank_0(f"{message} in {_format_elapsed(time.monotonic() - start_time)}{suffix}")
...
class _SaveProgressMonitor:
    ...
    def start(self) -> None:
        ...
        print_rank_0(f"Save progress monitor enabled for {self.checkpoint_root} (interval={self.interval_sec:.1f}s)")
        self._thread.start()
    ...
    def _emit_progress(self, *, final: bool = False) -> None:
        ...
        print_rank_0(f"[{label}] elapsed {elapsed} | files {file_count} | written {_format_num_bytes(total_bytes)}")
...
def parse_args(argv: list[str] | None = None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Allow custom model code execution (default: True; required for Kimi-K2.5 / DeepSeek-V3 family).",
    )
    parser.add_argument(
        "--debug-layer-range",
...
def convert_nvfp4_multigpu(
    ...
    *,
    ...
    print_rank_0(f"Multi-GPU NVFP4 conversion: {hf_model_path} -> {megatron_path}")
    print_rank_0(f"  TP={tp}  PP={pp}  EP={ep}  ETP={etp}  world_size={os.environ.get('WORLD_SIZE')}")
    ...
    trust_remote_code_resolved = getattr(auto_bridge.hf_pretrained, "trust_remote_code", False)
    tokenizer_kwargs = {"trust_remote_code": True} if trust_remote_code_resolved else None
    ...
    if debug_layer_range is not None:
        ...
        stage_start = _log_stage_start(f"Filtering conversion tasks for debug decoder layer range [{start}, {end})...")
    ...
    spill_prefix = f".{Path(megatron_path).name}.nvfp4_spill_rank{rank:04d}_"
    ...
    print_rank_0(f"Done. Direct NVFP4 Megatron checkpoint saved to: {megatron_path}")
    return 0
