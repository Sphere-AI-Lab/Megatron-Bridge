...
def main() -> int:
    ...
    if args.hf_model_path is None:
        raise SystemExit("--hf-model-path is required unless --probe-compress or --probe-roundtrip is set")
    ...
    with temporary_distributed_context(backend="gloo"):
        ...
        for module, keys in grouped.items():
            ...
            for k in keys:
                ...
                _emit(f"    {k}   type={type(entry).__name__}  dtype={dtype}  shape={shape}")
...
def _describe_tensor(tensor) -> str:
    ...
    return f"device={device} dtype={tensor.dtype} shape={tuple(tensor.shape)} numel={numel}{scalar_str}"
