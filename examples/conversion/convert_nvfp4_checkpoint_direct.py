...
def keep_meta_model_unmaterialized():
    ...
    ``to_empty_if_meta_device(model, device="cuda")`` after construction, which
    would try to allocate the full Megatron model in BF16 on GPU 0. For
    trillion-class checkpoints (Kimi-K2.5 NVFP4) that OOMs immediately. We only
    need module names, shapes, and sharded-state metadata for direct
    conversion — tensors are streamed straight from HF into the spill manager
    and the final NVFP4 buffers — so keeping the model purely on meta avoids
    every materialization step. Mirror of the same context manager in
...
def main():
    ...
    with temporary_distributed_context(backend="gloo"):
        ...
        # Spill is opt-in via env var. The default in-RAM path matches what the
        # INT4 direct converter does (it never spills) — Kimi-K2.5 NVFP4 fits
        # comfortably in B200-node RAM (~500 GB out of ~1.5-2 TB). Per-tensor
        # spill creates one mmap per tensor and exhausts the kernel's
        # vm.max_map_count (default 65530) at ~30% of conversion progress on
        # trillion-class models.
        use_spill = os.environ.get(
            "MEGATRON_BRIDGE_DIRECT_USE_SPILL", "0"
        ).lower() in ("1", "true", "yes")
        ...
        def _maybe_spill_manager():
            ...
            spill_parent = Path(
                os.environ.get(
                    "MEGATRON_BRIDGE_DIRECT_SPILL_DIR", str(checkpoint_parent)
                )
            ).resolve()
