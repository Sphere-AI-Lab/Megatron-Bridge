...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to NVFP4 Megatron checkpoint (converted via convert_nvfp4_checkpoint_direct.sh)",
    )
...
def main() -> None:
    ...
    output_dir = args.output_dir or os.path.join(
        os.getcwd(), "nemo_experiments", "kimi_k25_qoft_nvfp4"
    )
    ...
    config.logger.wandb_exp_name = (
        f"kimi_k25_qoft_nvfp4_bs{args.block_size}_tp{args.tp}_ep{args.ep}"
    )
