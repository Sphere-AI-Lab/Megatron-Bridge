...
"""
Qwen3-30B-A3B MoE OFT finetuning with FP8 (interactive, no SLURM).

OFT (Orthogonal Fine-Tuning) learns block-diagonal orthogonal rotations
applied to the input of linear layers: y = W @ R @ x.

Identical to finetune_oft.py (GPT-OSS) except:
  - Uses qwen3_30b_a3b_peft_config instead of gpt_oss_20b_peft_config
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Qwen3-30B-A3B MoE OFT finetuning with FP8",
    ...
    parser.add_argument(
        ...
        type=str,
        default="hopper",
        choices=["hopper", "blackwell", "none"],
        help="FP8 recipe: 'hopper' for FP8 current scaling (H100+), 'blackwell' for MXFP8 (B200+), 'none' for BF16 only",
    )
...
def main() -> None:
    ...
    # Create OFT — same defaults as finetune_oft.py (GPT-OSS)
    oft = OFT(
    ...
    # === Only difference from BF16 version: enable FP8 ===
    if args.fp8_recipe == "hopper":
        ...
        config.model.moe_router_padding_for_fp8 = True
    # "none" → keep BF16 default from recipe
    ...
    output_dir = args.output_dir or os.path.join(os.getcwd(), "nemo_experiments", "qwen3_30b_a3b_oft_fp8")
    ...
    config.logger.wandb_exp_name = f"qwen3_30b_a3b_oft_fp8_bs{args.block_size}_tp{args.tp}_ep{args.ep}"
