# Orbit fork layout: package + shims + seams

Contract of this fork (`Orbit-AI-Lab/Megatron-Bridge`, branch `orbit-main`):

- Everything wholly orbit-owned lives in `src/megatron/bridge/orbit/`:
...
- Launcher scripts live in `scripts/orbit/` (run from the repo root; the
...
- **Compatibility shims** (22 module files + the `models/conversion/low_precision/`
  ...
  aliases `sys.modules[old] = <orbit module>`, so old and new dotted paths
...
- Everything else that differs from upstream `NVIDIA-NeMo/Megatron-Bridge` is a
  ...
  functions carry a `# orbit-seam(<tag>):` marker.
...
|---|---|---|---|
| `models/conversion/peft_bridge.py` | +123/−29 | mixin + rewiring | `MegatronPeftBridge(OrbitOFTExportMixin)` base (methods live in `orbit/conversion/oft_export.py`); widened `_get_lora_unwrapped_name` / `_is_adapter_param_name` predicates; OFT-aware rewrites of `infer_target_modules_from_adapter_weights` / `build_adapter_config_dict` (+`_HF_OFT_SUFFIXES`) — upstream-unit-tested, LoRA-behavior-entangled, kept in place |
...
| `models/kimi_vl/utils.py` | +9/−123 | shim | INT4 helpers consolidated into `orbit/low_precision/int4.py`; file-path loaders depend on this shim |
...
| `models/qwen/qwen3_moe_bridge.py` | +12 | behavior | fp32 router dtype + `moe_layer_freq` derivation (inherited by orbit fp8/int4 bridges) |
...
| `training/checkpointing.py` | +18/−1 | hooks | async modelopt save + sharded-load restore call sites (impl in `orbit/training/modelopt_checkpoint.py`) |
| `training/post_training/checkpointing.py` | +6 | hook | packed-layout compress on restore (impl in `orbit/training/modelopt_packed_restore.py`); orbit imports `has_modelopt_state`/`load_modelopt_state` from here |
...
| `.gitmodules` + `3rdparty/Megatron-LM` | +3/−2 | pin | submodule → `Orbit-AI-Lab/Megatron-LM@orbit-main` |
...
- Duplicated helpers: `_module_bias_enabled`/`_get_active_bias_tensor` exist in
  `orbit/peft_ext/int4_lora_forward.py` and `orbit/oft/oft_layers.py`;
  `_compose_module_name` in `orbit/peft_ext/bias_normalization.py` and the DSV4
  bridge — dedupe inside the orbit package at leisure.
- `OrbitOFTExportMixin._merge_oft_adapter_weights` / `_compute_oft_rotation_matrix`
  have zero callers in bridge and orbit (reserved code).
- Pre-existing `ruff format` nonconformance in many orbit files (predates the
  restructure; the restructure deliberately did not reformat).

## Pulling upstream / porting to another fork

- **Upstream pull:** merge; `orbit/`, `scripts/orbit/`, examples, and the shims
  ...
  review rule.
- **Cross-fork port:** copy `orbit/` verbatim, re-apply the seams, and decide
