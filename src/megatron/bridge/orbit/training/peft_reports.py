...
def _write_parameter_partition_report(entries: list[dict[str, Any]], report_path: os.PathLike[str] | str) -> None:
    ...
    with open(report_path, "w", encoding="utf-8") as report_file:
        ...
        for entry in entries:
            report_file.write(f"{entry['name']}\t{entry['numel']}\t{entry['dtype']}\t{entry['shape']}\n")
...
def _write_peft_parameter_reports(
    ...
    total_params = sum(entry["numel"] for entry in trainable_entries) + sum(entry["numel"] for entry in frozen_entries)
    ...
    with open(summary_report, "w", encoding="utf-8") as summary_file:
        ...
        summary_file.write(f"frozen_percentage\t{(100 * frozen_params / total_params) if total_params else 0.0:.2f}\n")
