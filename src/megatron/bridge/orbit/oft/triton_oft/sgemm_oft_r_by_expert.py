...
def _oft_r_by_expert_fwd_kernel(
    x_ptr, out_ptr, R_ptr, offsets_ptr, counts_ptr,
    input_dim: tl.constexpr,
    x_stride_0, out_stride_0,
    R_stride_e, R_stride_b, R_stride_i, R_stride_j,
    BLOCK_M: tl.constexpr, BLOCK_SIZE: tl.constexpr, TILE_K: tl.constexpr,
):
    ...
    for k_off in range(0, BLOCK_SIZE, TILE_K):
        ...
        r_ptrs = (
            R_ptr + expert * R_stride_e + block_idx * R_stride_b
            + r_rows[:, None] * R_stride_i + col_offsets[None, :] * R_stride_j
        )
...
def _oft_r_by_expert_grad_R_kernel(
    x_ptr, grad_y_ptr, grad_R_ptr, offsets_ptr, counts_ptr,
    input_dim: tl.constexpr,
    x_stride_0, gy_stride_0,
    grad_R_stride_e, grad_R_stride_b, grad_R_stride_i, grad_R_stride_j,
    BLOCK_SIZE: tl.constexpr, TILE_T: tl.constexpr,
):
    expert = tl.program_id(0)
    ...
    out_ptrs = (
        grad_R_ptr + expert * grad_R_stride_e + block_idx * grad_R_stride_b
        + k_offsets[:, None] * grad_R_stride_i + c_offsets[None, :] * grad_R_stride_j
    )
...
def _fwd_kernel_launch(
    ...
    _oft_r_by_expert_fwd_kernel[grid](
        x, output, R, offsets, counts, input_dim,
        x.stride(0), output.stride(0),
        R.stride(0), R.stride(1), R_stride_i, R_stride_j,
        BLOCK_M=block_m, BLOCK_SIZE=block_size, TILE_K=tile_k,
    )
    return output


def oft_r_by_expert_fwd(
    x: torch.Tensor, R: torch.Tensor, tokens_per_expert: torch.Tensor,
) -> torch.Tensor:
...
def oft_r_by_expert_grad_R(
    x: torch.Tensor, grad_y: torch.Tensor,
    R_shape: tuple[int, int, int, int], tokens_per_expert: torch.Tensor,
    ...
    _oft_r_by_expert_grad_R_kernel[(num_experts, num_blocks)](
        x, grad_y, grad_R, offsets, counts, x.shape[1],
        x.stride(0), grad_y.stride(0),
        grad_R.stride(0), grad_R.stride(1), grad_R.stride(2), grad_R.stride(3),
        BLOCK_SIZE=block_size, TILE_T=tile_t,
    )
    return grad_R
...
class OFTRotationByExpertFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_x = _fwd_kernel_launch(
            grad_output, R, offsets, counts, max_count, R.stride(3), R.stride(2),
        )
        grad_R = oft_r_by_expert_grad_R(
            x, grad_output, tuple(R.shape), tokens_per_expert=None,
            offsets=offsets, counts=counts, max_count=max_count,
        ).to(R.dtype)
