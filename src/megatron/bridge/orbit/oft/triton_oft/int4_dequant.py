...
if _HAS_TRITON:
    ...
    def _int4_dequant_kernel(
        ...
        packed = tl.load(
            w_packed_ptr + offs_n[:, None] * stride_wn + (offs_k[None, :] // 8) * stride_wkpack,
            mask=(offs_n[:, None] < N) & (offs_k[None, :] < K),
            other=0,
        )
        scale = tl.load(
            w_scale_ptr + offs_n[:, None] * stride_sn + (offs_k[None, :] // GROUP_SIZE) * stride_sg,
            mask=(offs_n[:, None] < N) & (offs_k[None, :] < K),
            other=0.0,
        ).to(tl.float32)
...
def dequantize_int4_triton(
    ...
    *,
    ...
    if scale.shape[1] != expected_groups:
        raise ValueError(
            f"expected {expected_groups} scale groups for shape ({out_features}, {in_features}), got {scale.shape[1]}"
        )
