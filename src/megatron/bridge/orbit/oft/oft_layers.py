...
def _get_oft_fp8_activation_quant_mode() -> str:
    ...
    if mode in ("w8a8", "fp8", "dynamic"):
        return "w8a8"
    raise ValueError(f"Unsupported MEGATRON_OFT_FP8_ACTIVATION_QUANT={mode!r}. Expected one of: none, w8a8.")
...
def _fp8_activation_qdq_per_token_group_ste(
    ...
    if x.shape[-1] % group_size != 0:
        raise ValueError(
            f"FP8 W8A8 activation quantization requires hidden size divisible by {group_size}, got {x.shape[-1]}."
        )
...
def set_int4_grouped_chunk_backend(backend: str) -> None:
    ...
    if normalized not in ("python", "te"):
        raise ValueError(f"Unsupported INT4 grouped chunk backend {backend!r}. Expected one of: python, te.")
...
try:
    from megatron.bridge.orbit.oft.triton_oft import cayley_neumann as triton_cayley_neumann
    from megatron.bridge.orbit.oft.triton_oft import oft_r_single as triton_oft_r_single
...
class OFTLinear(AdapterWrapper):
    ...
    def __init__(self, to_wrap: nn.Module, adapter: OFTRotationModule):
        ...
        first_scale_inv = (
            getattr(self.to_wrap, f"{self._weight_names[0]}_scale_inv", None) if self._weight_names else None
        )
    ...
    def _nvfp4_grouped_scale_suffix(self, local_suffix: str) -> Optional[str]:
        ...
        scale_suffixes = {match.group(1) for name in buffer_names if (match := scale_re.match(name)) is not None}
        double_suffixes = {match.group(1) for name in buffer_names if (match := double_re.match(name)) is not None}
    ...
    def _debug_nvfp4_detection_miss(self, x: torch.Tensor) -> None:
        ...
        if quantizer is not None:
            quantizer_attrs = [
                name for name in sorted(dir(quantizer)) if name.startswith(("_scale", "_double_scale", "_amax"))
            ][:32]
        ...
        first_name = self._weight_names[0]
        has_scale = (
            self._get_nvfp4_quantizer_buffer(quantizer, "_scale", first_name) is not None if quantizer else False
        )
    ...
    def _get_nvfp4_scale_2(self, quantizer: nn.Module, name: str) -> Optional[torch.Tensor]:
        ...
        from megatron.bridge.orbit.low_precision.nvfp4 import NVFP4_AMAX_SCALE
    ...
    def _forward_nvfp4_grouped_buffers(
        ...
        if len(args) == 0:
            raise ValueError("Grouped OFTLinear NVFP4 forward requires tokens_per_expert as the first positional arg")
        ...
        if total_tokens == x.shape[0]:
            ...
            align = 16
            padded_splits = [((s + align - 1) // align) * align if s > 0 else 0 for s in token_splits]
    ...
    def _get_or_create_int4_te_chunk_module(
        ...
        *,
        ...
        logger.debug(
            "Enabled TE chunked INT4 grouped path for %s with num_gemms=%d, mode=%s, tp_size=%d, device=%s, dtype=%s",
            type(self.to_wrap).__name__,
            num_gemms,
            parallel_mode,
            tp_size,
    ...
    def _forward_int4(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        """
        if getattr(self.to_wrap, "num_gemms", 0) > 0 and self._int4_active_expert_chunk_size > 0 and len(args) > 0:
