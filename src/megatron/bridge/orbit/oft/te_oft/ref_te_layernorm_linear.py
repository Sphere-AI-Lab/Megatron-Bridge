        ...
        with get_nvtx_range_context("_LayerNormLinear_backward"):
            ...
            if ctx.requires_wgrad:
                ...
                if ctx.ub_overlap_ag and isinstance(ctx.grad_output_quantizer, MXFP8Quantizer):
                    ...
                    tex.bulk_overlap_ag_with_external_gemm(ub_obj_overlap_wgrad, dgrad_send_stream, dgrad_recv_stream)
...
class LayerNormLinear(TransformerEngineBaseModule):
    ...
    def __init__(
        ...
        self.return_layernorm_output_gathered = return_layernorm_output_gathered if return_layernorm_output else False
        ...
        self.ub_overlap_ag_fprop = ub_overlap_ag and self.sequence_parallel and self.parallel_mode == "column"
        self.ub_overlap_rs_dgrad = ub_overlap_rs_dgrad and self.sequence_parallel and self.parallel_mode == "column"
        self.ub_bulk_wgrad = (
            ub_bulk_wgrad
        ...
        self.ub_overlap_rs_fprop = ub_overlap_rs and self.sequence_parallel and self.parallel_mode == "row"
        self.ub_overlap_ag_dgrad = ub_overlap_ag and self.sequence_parallel and self.parallel_mode == "row"
        if any(
            [
        ...
        self.eps = eps
        layer_norm_weight = torch.nn.Parameter(torch.empty(self.in_features, device=device, dtype=params_dtype))
        ...
        if self.normalization != "RMSNorm":
            layer_norm_bias = torch.nn.Parameter(torch.empty(self.in_features, device=device, dtype=params_dtype))
            self.register_parameter("layer_norm_bias", layer_norm_bias, init_fn=init_method_constant(0.0))
        else:
        ...
        if sum(self.parameter_split_sizes) != out_features:
            raise ValueError(
                f"Trying to split weight buffer ({out_features=}) with split sizes {self.parameter_split_sizes}"
            )
    ...
    def forward(
        ...
        if self.ub_overlap_rs_fprop:
            if get_ub(self.ub_name + "_fprop", FP8GlobalStateManager.is_fp8_enabled()).is_fp8_ubuf():
                fp8_output = True
        if self.ub_overlap_rs_dgrad:
            if get_ub(self.ub_name + "_dgrad", FP8GlobalStateManager.is_fp8_enabled()).is_fp8_ubuf():
                fp8_grad = True
    ...
    def _get_debug_quantizers(self, fp8_output, fp8_grad, is_grad_enabled):
        ...
        return tuple(DebugQuantizer(self.name, name, q, self.tp_group) for name, q in zip(names, original_quantizers))
    ...
    def _get_weight_tensors(self) -> List[Union[torch.Tensor, QuantizedTensorStorage]]:
        ...
        if any(isinstance(w, QuantizedTensor) for w in unfused_weights):
            if self.fp8:
                ...
                warnings.warn(
                    "You are using quantized weights without quantized compute. Please make sure this is intentional."
                )
