...
try:
    from megatron.bridge.sphere.oft.triton_oft import cayley_neumann as triton_cayley_neumann
    from megatron.bridge.sphere.oft.triton_oft import oft_r_single as triton_oft_r_single
    HAS_TRITON_OFT = True
except ImportError:
...
from megatron.bridge.sphere.quant.int4_utils import _empty_storage_view
from megatron.bridge.sphere.quant.qwen3_fp8_gemm import (
...
class OFTLinear(AdapterWrapper):
    ...
    def _get_nvfp4_scale_2(self, quantizer: nn.Module, name: str) -> Optional[torch.Tensor]:
        ...
        from megatron.bridge.sphere.low_precision.nvfp4 import NVFP4_AMAX_SCALE
    ...
    def _nvfp4_weight_shape(self, packed: torch.Tensor, scale: torch.Tensor, name: str = "weight") -> torch.Tensor:
        ...
        if getattr(scale, "ndim", 0) >= 2:
            from megatron.bridge.sphere.low_precision.nvfp4 import NVFP4_GROUP_SIZE
    ...
    def _forward_nvfp4_te_linear(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        from megatron.bridge.sphere.low_precision.nvfp4 import dequantize_nvfp4
    ...
    def _forward_fp8_te_linear(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        NVFP4 fallbacks.
        """
        from megatron.bridge.sphere.quant.fp8_utils import dequant_fp8
