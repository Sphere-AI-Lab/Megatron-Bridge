...
from megatron.bridge.sphere.oft.oft import _SplitLNOFTLinear
...
try:
    from megatron.bridge.sphere.oft.triton_oft import oft_r_by_expert
except ImportError:
...
def _has_dequantized_nvfp4_buffers(module: nn.Module) -> bool:
    return hasattr(module, "_nvfp4_weight_scale") and hasattr(
        module, "_nvfp4_weight_double_scale"
    )
...
def _stack_oft_r_for_tp(modules: list[OFTRotationModule]) -> torch.Tensor:
    ...
    for module in modules:
        if module.input_is_parallel:
            ...
            stacked.append(
                copy_to_tensor_model_parallel_region(module.oft_r, group=module.tp_group)
            )
...
def _batched_equal_output_linear_with_bias(
    ...
    if W_stack.shape[0] != num_slices:
        raise ValueError(
            f"x_stack and W_stack disagree on num_slices: {x_stack.shape[0]} vs {W_stack.shape[0]}"
        )
    K = x_stack.shape[-1]
...
class OFTLinearSplitFC1UpGate(nn.Module):
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any):
        if _is_quantized_single_weight_linear(self.to_wrap):
            return _run_quantized_shared_oft_fallback(
                self, self.adapter_gate, x, *args, **kwargs
            )
...
class GroupedOFTRotation(nn.Module):
    ...
    def __init__(
        ...
        if num_local_experts <= 0:
            raise ValueError(
                f"GroupedOFTRotation requires num_local_experts > 0, got {num_local_experts}"
            )
    ...
    def __getitem__(self, expert_idx: int) -> "_PerExpertOFTRotationView":
        ...
        if not (0 <= expert_idx < self.num_local_experts):
            raise IndexError(
                f"expert_idx={expert_idx} out of range for "
    ...
    def compute_rotation_bank(self) -> torch.Tensor:
        ...
        if self.input_is_parallel:
            ...
            oft_r_parallel = copy_to_tensor_model_parallel_region(
                self.oft_r, group=self.tp_group
            )
    ...
    def forward(self, x: torch.Tensor, expert_idx: int) -> torch.Tensor:
        ...
        if self.input_is_parallel:
            ...
            oft_r = copy_to_tensor_model_parallel_region(
                self.oft_r[expert_idx], group=self.tp_group
            )
        ...
        rank = (
            self.in_features // self.block_size if self.block_share else self.r
        )
        if self.block_share:
            R = R.repeat(rank, 1, 1)
        orig_shape = x.shape
...
class OFTLinearGroupedSplitFC1UpGate(nn.Module):
    ...
    def __init__(
        ...
        if self.num_gemms <= 0:
            raise ValueError(
                f"{type(self).__name__} requires a grouped expert module with num_gemms > 0"
            )
    ...
    def _assert_unquantized(self) -> None:
        ...
        if (
            getattr(first_weight, "dtype", None) == torch.uint8
            or hasattr(self.to_wrap, "weight0_w_packed")
        ):
            raise RuntimeError(
    ...
    def _forward_int4_split_eager(
        ...
        with torch.autograd.graph.saved_tensors_hooks(pack, unpack):
            for expert_idx, token_count in enumerate(tokens_per_expert):
                ...
                w_compute = dequantize_int4(
                    packed, scale, shape, device=gate_x.device
                ).to(gate_x.dtype)
    ...
    def _get_or_create_te_grouped_half_module(
        ...
        *,
        ...
        logger.debug(
            "Enabled TE grouped split-OFT half GEMM for %s side=%s num_gemms=%d "
            "input=%d output=%d device=%s dtype=%s",
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any):
        ...
        if len(args) == 0:
            raise ValueError(
                f"{type(self).__name__} requires tokens_per_expert as the first positional argument"
            )
        tokens_per_expert = self._normalize_tokens_per_expert(args[0])
        if len(tokens_per_expert) != self.num_gemms:
            raise ValueError(
                f"Expected {self.num_gemms} token splits for grouped FC1, "
                f"got {len(tokens_per_expert)}"
            )
        if sum(tokens_per_expert) != x.shape[0]:
            raise ValueError(
                f"tokens_per_expert sums to {sum(tokens_per_expert)}, "
                f"but grouped FC1 input has {x.shape[0]} rows"
            )
        ...
        if self._can_use_te_grouped_half_gemm(gate_x, up_x):
            try:
                return self._forward_te_grouped_half_gemm(
                    gate_x, up_x, tokens_per_expert
                )
            except Exception as exc:
                logger.warning(
                    "Falling back to eager grouped split-OFT GEMM for %s "
                    "because TE grouped half-GEMM failed: %s",
                    type(self.to_wrap).__name__,
                    exc,
                )
...
class OFTLinearSplitQKV(nn.Module):
    ...
    def _packed_qkv_layout(self, packed_dim: int) -> tuple[int, int, int]:
        ...
        if packed_dim % head_size != 0:
            raise ValueError(
                f"linear_qkv packed dim {packed_dim} must be divisible by head_size={head_size}"
            )
    ...
    def _fused_fast_path_supported(self) -> bool:
        ...
        return _oft_fast_path_supported(
            [self.adapter_q, self.adapter_k, self.adapter_v]
        )

    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any):
        if _is_quantized_single_weight_linear(self.to_wrap):
            return _run_quantized_shared_oft_fallback(
                self, self.adapter_q, x, *args, **kwargs
            )
        self._assert_unquantized()
        W = self.to_wrap.weight
        ...
        if self._fused_fast_path_supported():
            ...
            if W_k.shape[0] == W_v.shape[0]:
                ...
                kv_out = _batched_equal_output_linear_with_bias(
                    x_stack[1:3], W_kv_stack, None
                )
    ...
    def transform(self, module: nn.Module, name: Optional[str] = None, prefix: Optional[str] = None) -> nn.Module:
        ...
        if matched_pattern == "word_embeddings":
            ...
            assert embedding_dim is not None, (
                f"Cannot infer embedding_dim from {type(module).__name__} at {full_name}"
            )
            adapter = OFTRotationModule(
        ...
        if matched_pattern.endswith("linear_fc1") and _should_treat_linear_fc1_as_unfused(full_name):
            logger.info("CanonicalOFT: OFTLinear at %s (treating unsupported canonical linear_fc1 as unfused)", full_name)
