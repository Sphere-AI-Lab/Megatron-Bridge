...
_TE_OFT_ATTN_DEBUG_EMITTED = False


def _debug_mask_type_name(mask_type: Any) -> str:
    return getattr(mask_type, "name", str(mask_type))


def _maybe_emit_te_oft_attention_debug(
    *,
    rank: Optional[int],
    attn_mask_type: Any,
    module_attn_mask_type: Any,
    qkv_format: str,
    te_forward_mask_type: bool,
    attention_mask_is_none: bool,
    packed_seq_present: bool,
    cu_seqlens_q_present: bool,
    cu_seqlens_q_padded_present: bool,
    emit: Optional[Callable[[str], None]] = None,
) -> bool:
    global _TE_OFT_ATTN_DEBUG_EMITTED

    if os.getenv("MBRIDGE_OFT_TE_ATTN_DEBUG", "0") != "1":
        return False
    if _TE_OFT_ATTN_DEBUG_EMITTED or qkv_format != "thd":
        return False

    rank_text = "unknown" if rank is None else str(rank)
    message = (
        "MBRIDGE_TE_OFT_ATTN_DEBUG "
        f"rank={rank_text} "
        f"attn_mask_type={_debug_mask_type_name(attn_mask_type)} "
        f"module_attn_mask_type={_debug_mask_type_name(module_attn_mask_type)} "
        f"qkv_format={qkv_format} "
        f"te_forward_mask_type={te_forward_mask_type} "
        f"attention_mask_is_none={attention_mask_is_none} "
        f"packed_seq_present={packed_seq_present} "
        f"cu_seqlens_q_present={cu_seqlens_q_present} "
        f"cu_seqlens_q_padded_present={cu_seqlens_q_padded_present}"
    )
    _TE_OFT_ATTN_DEBUG_EMITTED = True
...
class TEQuantizationRecipe:
    ...
    def parse_from_config(cls, quant_config: Dict[Any, Any]) -> "TEQuantizationRecipe":
        ...
        if instance.fp8_quantization_recipe is not None and instance.fp4_quantization_recipe is not None:
...
class TEQuantizationParams:
    ...
    def parse_from_config(quant_config: QuantizationConfig) -> "TEQuantizationParams":
        ...
        try:
            ...
            raise ValueError(f"TransformerEngine config dictionary must have '{_TE_CONFIG_TYPE_KEY}' key.")
        except ValueError:
        ...
        if config_type == TransformerEngineConfigType.TEQuantizationParams:
            if "training_recipe" not in config.keys():
                raise ValueError("TransformerEngine config dictionary must have 'training_recipe' key")
            ...
            if "evaluation_recipe" not in config.keys():
                ...
                evaluation_recipe = TEQuantizationRecipe.parse_from_config(config["evaluation_recipe"])
                assert len(config.keys()) == 3
            return TEQuantizationParams(training_recipe=training_recipe, evaluation_recipe=evaluation_recipe)
        else:
...
def _get_fp8_autocast_for_quant_recipe(qrecipe: TEQuantizationRecipe):
    ...
    if qrecipe.fp8_quantization_recipe is None and qrecipe.fp4_quantization_recipe is None:
        ...
        if qrecipe.fp8_quantization_recipe == Fp8Recipe.custom or qrecipe.fp4_quantization_recipe == Fp4Recipe.custom:
...
def _get_should_context_be_quantized_recipe(qrecipe: TEQuantizationRecipe, is_original_context_quantized: bool):
...
def _get_should_context_be_quantized_params(
    ...
    if qparams is None:
        ...
        return _get_should_context_be_quantized_recipe(qparams.evaluation_recipe, is_context_quantized)
    else:
        return _get_should_context_be_quantized_recipe(qparams.training_recipe, is_context_quantized)
                    ...
                    if isinstance(fused_layer.fp8_meta[key], dict):
                        for subkey in fused_layer.fp8_meta[key]:
                            if subkey in linear_layer.fp8_meta[key]:
                                linear_layer.fp8_meta[key][subkey] = fused_layer.fp8_meta[key][subkey]
                    else:
...
class TELinear(te.pytorch.Linear):
    ...
    def __init__(
        ...
        *,
        ...
        if symmetric_ar_type is not None:
            ...
            assert is_te_min_version("2.3.0") or get_te_version() == PkgVersion("2.3.0.dev0+39c0e70"), (
    ...
    def will_execute_quantized(self, is_context_quantized: bool) -> bool:
        ...
        return _get_should_context_be_quantized_params(self.te_quant_params, self.training, is_context_quantized)
    ...
    def sharded_state_dict(self, prefix="", sharded_offsets=(), metadata=None):
        ...
        assert self.parallel_mode is None, "TELinear sharded_state_dict can only be used with duplicated parallel mode"
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            get_rng_state_tracker=(get_cuda_rng_tracker if get_cuda_rng_tracker().is_initialized() else None),
            init_method=(
                condition_init_method(config, init_method) if not config.use_cpu_initialization else lambda w: None
            ),
            bias=bias,
            return_bias=self.te_return_bias,
...
class TEDotProductAttention(te.pytorch.DotProductAttention):
    ...
    def __init__(
        ...
        self.num_splits: Optional[int] = 1 if (num_splits is None and self.config.batch_invariant_mode) else num_splits
    ...
    def forward(
        ...
        if packed_seq_params is not None:
            ...
            # Indicates to turn off CP dynamically
                ...
                assert packed_seq_params.local_cp_size == 1, "local_cp_size must be == 1 if provided without cp_group"
        ...
        _maybe_emit_te_oft_attention_debug(
            rank=rank,
            attn_mask_type=attn_mask_type,
            module_attn_mask_type=getattr(self, "attn_mask_type", None),
            qkv_format=qkv_format,
            te_forward_mask_type=self.te_forward_mask_type,
            attention_mask_is_none=attention_mask is None,
            packed_seq_present=packed_seq_params is not None,
            cu_seqlens_q_present=getattr(packed_seq_params, "cu_seqlens_q", None) is not None,
            cu_seqlens_q_padded_present=getattr(packed_seq_params, "cu_seqlens_q_padded", None) is not None,
        )
        ...
        if self.te_forward_mask_type:
            ...
            _fa_kwargs = dict(attn_mask_type=attn_mask_type.name, **attention_bias_kwargs, **packed_seq_kwargs)
            if num_splits is not None:
            ...
            if self.config.qk_clip or self.config.log_max_attention_logit:
                ...
                if self.current_max_attn_logits is None:
                    ...
                    self.current_max_attn_logits = torch.max(self.current_max_attn_logits, batch_max_attention_logits)
        ...
        def __init__(
            ...
            *,
            ...
            def merge_extra_states(
                ...
                try:
                    state_list = [state_dict.pop(f"{prefix}_extra_state{i}") for i in range(1, self.num_gemms)]
                except KeyError:
                ...
                sharded_state_dict.update(
                    {
                        ...
                        f"{prefix}_extra_state{'' if gemm_idx == 0 else gemm_idx}": sub_sd[f"{gemm_idx}._extra_state"],
                    }
                )
                if self.use_bias:
    ...
    class TEFusedMLP(MLP):
        ...
        def _register_hooks_on_fused_impl(self, fused_impl: torch.nn.Module) -> None:
            ...
            if forward_pre_hooks:
                ...
                if any(inspect.getmodule(hook) != distributed_data_parallel for _, hook in forward_pre_hooks):
                    warnings.warn(
            ...
            if backward_pre_hooks:
                raise RuntimeError("TEFusedMLP module does not support submodules with pre-backward hooks")
            if backward_post_hooks:
                raise RuntimeError("TEFusedMLP module does not support submodules with post-backward hooks")
...
try:
    ...
    def fused_apply_rotary_pos_emb(
        ...
        if transpose_output_memory:
            warnings.warn("transpose_output_memory is not supported by TE's fused RoPE and will be ignored.")
        if is_te_min_version("2.3.0"):
            return apply_rotary_pos_emb(t, freqs, tensor_format="sbhd", interleaved=interleaved, fused=True)
        else:
            if interleaved:
    ...
    def fused_apply_rotary_pos_emb_thd(
        ...
        if is_te_min_version("1.12.0", check_equality=True):
            ...
            return apply_rotary_pos_emb(t, freqs, tensor_format="thd", fused=True, cu_seqlens=cu_seqlens)
...
try:
    ...
    def te_parallel_cross_entropy(
        ...
        if _TE_SUPPORTS_CG_CAPTURABLE:
            ...
            return parallel_cross_entropy(logits, labels, 0.0, False, tp_group, -100, is_cg_capturable)
        else:
