...
def _model_key_filter(key: str) -> bool:
    return key == "model" or key.startswith("model.") or (
        key.startswith("model") and len(key) > 5 and key[5].isdigit() and "." in key
    )
...
def _logical_numel_from_quantized_entry(
    ...
    if spec.scale_group_size is not None and len(shape) == 2 and (
        key.endswith(".weight_scale") or key.endswith("_scale")
    ):
...
def summarize_logical_tensor_collection(node: Any, *, chunk_size: int = 128) -> LogicalTensorSummary:
    items = [
        ...
        for key, tensor in iter_named_tensors(node, chunk_size=chunk_size)
    ]
...
def compare_tensor_summaries(
    ...
    if hf_summary.total_numel != megatron_summary.total_numel:
        mismatches.append(
            f"total numel mismatch: hf={hf_summary.total_numel}, "
            f"megatron={megatron_summary.total_numel}"
        )
...
def compare_logical_tensor_summaries(
    ...
    if hf_summary.total_numel != megatron_summary.total_numel:
        mismatches.append(
            f"total logical numel mismatch: hf={hf_summary.total_numel}, "
            f"megatron={megatron_summary.total_numel}"
        )
    ...
    for category in sorted(hf_categories | megatron_categories):
        ...
        if hf_category.numel != megatron_category.numel:
            mismatches.append(
                f"logical numel mismatch for {category}: "
                f"hf={hf_category.numel}, megatron={megatron_category.numel}"
            )
...
def compare_config_fields(hf_fields: Mapping[str, Any], megatron_fields: Mapping[str, Any]) -> list[str]:
    ...
    for field_name in sorted(set(hf_fields) & set(megatron_fields)):
        if hf_fields[field_name] != megatron_fields[field_name]:
            mismatches.append(
                f"config mismatch for {field_name}: "
                f"hf={hf_fields[field_name]}, megatron={megatron_fields[field_name]}"
            )
